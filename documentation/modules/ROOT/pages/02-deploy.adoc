...
Run the following command to see how kustomize parse `YAML` files. This example does not apply any configuration, in order to apply it use `oc appl -k` or `oc apply --kustomize=''` to pocess a kustomization directory
