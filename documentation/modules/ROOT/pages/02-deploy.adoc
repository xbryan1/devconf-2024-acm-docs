...
[#managedcluster]
== Add Openshift cluster to a ACM ClusterSet

After the ACM/GitOps integration is done, you label `local-cluster` (ACM) with `ClusterSet rhte2023-gitops-clusters` and environment `development`. This configuration will let us deploy applications into this Openshift cluster.
...
and check that cluster `local-cluster` is added as an ArgoCD Cluster from the ArgoCD Console > Settings > Cluster
